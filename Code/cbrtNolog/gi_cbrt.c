...
// The original assumption of Langdon and Petke was that loglike helps with really small diffs in the fitness function
// to valdiate or reject this hypothesis this is the ONLY change compared to the code in the cbrt folder removing the log
double loglike(const double diff) {
  return diff; // no need for fabs as all values put in here are already positive
